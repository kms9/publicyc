...
func RawConfig(key string) *Config {
	...
	if conf.Detail().Get(key) == nil {
		onion_log.Panicf("key:%s pgConfig is not exists", key)
	}
	if err := conf.Detail().UnmarshalKey(key, config); err != nil {
		onion_log.Panicf("unmarshal pgConfig key:%s err:%s", key, err)
	}
...
func (c *Config) Build() *DB {
	...
	db, err := Open(c)
	if err != nil {
		c._logger.Panicf("connect pg err %s", err)
	}

	if err := db.DB().Ping(); err != nil {
		c._logger.Panicf("ping pg err %s", err)
	}
	return db
}
