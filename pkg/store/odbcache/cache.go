...
func (t *TableCache) parseValue(v []byte) interface{} {
	...
	case int64: // 如果查询的类型是 map查询条件之后 总的条目数量
		...
		if err := json.Unmarshal(v, &count); err != nil {
			onion_log.Errorf("TableCache parseValue int64 err:%s v:%s", err, string(v))
			return nil
		}
		return count
	...
	case []string: // 如果查询的类型是 map查询条件之后 条目的主键id
		ids := make([]string, 0)
		if err := json.Unmarshal(v, &ids); err != nil {
			onion_log.Errorf("TableCache parseValue []string err:%s v:%s", err, string(v))
			return nil
		}
		return ids
	case []interface{}: // 如果查询的类型是 map查询条件之后 条目的主键id
		...
		case "string":
			ids := make([]string, 0)
			if err := json.Unmarshal(v, &ids); err != nil {
				onion_log.Errorf("TableCache parseValue []interface{} []string err:%s v:%s", err, string(v))
				return nil
			}
			return ids
		case "int64":
			ids := make([]int64, 0)
			if err := json.Unmarshal(v, &ids); err != nil {
				onion_log.Errorf("TableCache parseValue []interface{} []int64 err:%s v:%s", err, string(v))
				return nil
			}
			return ids
		}
